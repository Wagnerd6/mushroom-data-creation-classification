...
def data_secondary():
    return pd.read_csv(FILE_PATH_SECONDARY_GENERATED,
                       sep=';', header=0, na_values='?', low_memory=False)


def data_original():
    return pd.read_csv(FILE_PATH_1987,
                       sep=',', header=0, dtype=object, na_values='?')

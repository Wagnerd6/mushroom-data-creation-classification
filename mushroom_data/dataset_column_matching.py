...
def find_different_named_columns_list(*datas):
...
def get_datasets_with_identical_columns(data1, data2, **kwargs):
    ...
    if kwargs['encode']:
        datas.append(mushroom_classifier.encode_data_numerical(data1.drop(drop_categories[0], 1)))
        datas.append(mushroom_classifier.encode_data_numerical(data2.drop(drop_categories[1], 1)))
...
def get_encoded_column_matchings_dataframe(data1, data2, **kwargs):
    ...
    **kwargs:
    ...
    datas_matched_encoded = [mushroom_classifier.encode_data_numerical(datas_matched[0]),
                             mushroom_classifier.encode_data_numerical(datas_matched[1])]
...
if __name__ == "__main__":
    ...
    data_new = pd.read_csv(dataset_categories.FILE_PATH_SECONDARY_NO_MISS, sep=';', header=0, na_values='?', low_memory=False)
    data_og = pd.read_csv(dataset_categories.FILE_PATH_1987_NO_MISS, sep=';', header=0, dtype=object, na_values='?')
    ...
    matching_dataframe.to_csv(dataset_categories.FILE_PATH_COLUMN_MATCHING, sep=';', index=False)

    # encode datasets
    data_new = mushroom_classifier.encode_data_numerical(data_new)
    data_og = mushroom_classifier.encode_data_numerical(data_og)

    # rename encoded columns according to dataset_variable_encoding_mapping_edited.csv
    data_columns_encoded_mapping = pd.read_csv(dataset_categories.FILE_PATH_COLUMN_MATCHING_EDITED, sep=';', header=0)
    ...
    data_og.to_csv(dataset_categories.FILE_PATH_1987_MATCHED, sep=';', index=False)
    data_new.to_csv(dataset_categories.FILE_PATH_SECONDARY_MATCHED, sep=';', index=False)
